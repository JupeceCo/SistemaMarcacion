...
document.addEventListener("DOMContentLoaded", ()=>{
	const manual = document.querySelector("#manual").addEventListener("click", agregarManual);
    const auto = document.querySelector("#auto").addEventListener("click", agregarAuto);
});


function agregarManual() {
	activityBox.innerHTML = `<input type="text" class="form-control" placeholder="Ingrese una actividad" style="width: 100%; text-transform: uppercase;" id="actividad" name="states[]"> </input> `;
}

function agregarAuto(){
	activityBox.innerHTML = `<select class="form-control select2" style="width: 100%; text-transform: uppercase;" id="actividad" name="states[]" multiple="multiple"></select>`;
	listadoActividades();
	$(".select2").select2();
}
      ...
      for (let i = 1; i <= ultimoDia.getDate(); i++) {
        ...
        if (i == 1) {
          for (let j = 0; j < diainicio; j++) {
            fila +=
              '<td style="padding: 0" ><div style="margin: 0;" class="btn btn-app col-12 button-calendar" ></div></td>';
          }
          for (let j = diainicio; j <= 6; j++) {
            fila +=
              '<td style="padding: 0"> <div style="margin: 0;" class="btn btn-app col-12 button-calendar"> <span class="badge bg-danger" style="right: -1px; top: -1px">' +
              _datares[i].turno +
              "</span> " +
              Number(i).toFixed(0);
            if (_datares[i].clase != "") {
      ...
      for (let i = 1; i <= ultimoDia.getDate(); i++) {
        ...
        if (i == 1) {
          ...
          for (let j = diainicio; j <= 6; j++) {
            if (_datares[i].turno ) {
              fila +=
              '<td style="padding: 0"> <div style="margin: 0;" class="btn btn-app col-12 button-calendar" data-toggle="modal"    data-target="#exampleModalForm" onclick="seleccionardia(' +
            ...
            fila +=
              '<td style="padding: 0"> <div style="margin: 0;" class="btn btn-app col-12 button-calendar" data-toggle="modal" )"> '+Number(i).toFixed(0);;
  ...
  $.ajax({
    ...
    success: function (r) {
      if (r.success == true) {
        ...
        _datares.forEach((elemento, indice) => {
          $("#cmbpersonal").append(
            '<option value="' +elemento.nro + '" >' + elemento.descripcion +  "</option>"
          );
        });
        listadoServicio();
  ...
  $.ajax({
    ...
    success: function (r) {
      if (r.success == true) {
        ...
        msgbox("Las actividades se registraron correctamente.");
...
function listadoProgramadosDia(_Servicio) {
  ...
  // console.log("datlistadoprogramadodia",dat);
  $.ajax({
    data: dat,
    url: g_url + "apicalendar/api/horario.php",
    type: "POST",
    ...
    processData: false,
    success: function (r) {
		// console.log("response tabla tbody ", r)
      if (r.success == true) {
        let _datares = r.dataResult;
        _datares.forEach((elemento, indice) => {
          if (elemento.Autorizado == 0) {
...
function eliminarActividad(nro) {
  ...
  Swal.fire({
    title: '¿Deseas eliminar las actividades?',
  ...
  //     $("#exampleModalForm").modal("hide");
  //     Swal.fire('No se eliminó la actividad', '', 'info')
  //   }
  // })
      ...
      if (r.success == true) {
        ...
        _datares.forEach((elemento, indice) => {
          $("#lstservicioprogramados").append(
            '<div style="margin: 0;" class="external-event mb-1 ' +
              elemento.clase +
