    ...
    success: function (r) {
      if (r.success == true) {
        if (r.dataResult.length >= 0) {
          ...
          msgboxerror("No se encontraron registros...");
        }
      } else {
        msgboxerror("Error...", r.mensaje);
      }
...
function validar_conformidad(id){
  // let g_idtransaccion = localStorage.idtransaccion;
  // var dat = JSON.stringify({
  //   proceso: 5,
  //   p1: `${id}-${g_idtransaccion}`
  // });
...
function consultaReporteActividad() {
  ...
  if($("#cmbservicio").val() == ""){
    msgboxerror("Seleccione un servicio...");
    return;
  }

  if($("#cmbarea").val() == ""){
    msgboxerror("Seleccione una área...");
    return;
  }

  if($("#cmbpersonal").val() == ""){
    msgboxerror("Seleccione un personal...");
    return;
  }
      ...
      if (r.success == true) {
        ...
        if (r.dataResult.length >= 1) {
          ...
          _datares.forEach((elemento, indice) => {
            ...
            if( profile == "5") {
              var boton= `<button type="button" class="btn btn-warning" id="validar-admin">Validación del administrador</button>`;
            } else {
              if (elemento.idestado == 0 ) {
                var boton= `<button type="button" class="btn btn-info" onclick="validar_conformidad(`+elemento.idrol+`)">Validar Conformidad</button>`;
                }else if (elemento.idestado == 1) {
          ...
          msgboxerror(`No se encontraron registros de actividades del personal seleccionado...`);
        }
      } else {
        msgboxerror("Error...", r.mensaje);
      }
...
function listar() {
  tabla = $('#example2').dataTable({
    language: {
      ...
      const boton_admin = document.querySelectorAll("#validar-admin");
      if(boton_admin){
        boton_admin.forEach(boton=>{
          boton.addEventListener("click", ()=>{
            msgboxerror("Prohibido", "La validación debe ser realizada por un administrador.");
          });
        });
      }
}
