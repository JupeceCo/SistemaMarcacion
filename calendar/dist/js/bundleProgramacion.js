...
function cargarcalendar() {
	...
	let fechaini = g_periodo + "-" + nromes + "-02";
	let date = new Date(fechaini);
	let primerDia = new Date(date.getFullYear(), date.getMonth(), 1); // Halla el primer día del mes, Sat Jan 01 2022 00:00:00
	let ultimoDia = new Date(date.getFullYear(), date.getMonth() + 1, 0); // Halla el último día del mes, Mon Jan 31 2022 00:00:00 GMT-0500 (hora estándar de Perú)
	let diainicio = date.getDay(); // 0 domingo - 6 sábado
	let contadordias = 1;
	let datafecha = [];
	let columna = ""; 
	let fila = "";
	...
	$.ajax({
		...
		for (let i = 1; i <= ultimoDia.getDate(); i++) { //getDate obtiene el número de días del mes
			fila = "";
			//columna = "";
			columna = "<tr>"; 
			...
			if (i == 1) {
				...
				for (let j = 0; j < diainicio; j++) { //Para llenar con cuadros vacíos los días que no son del mes actual
					fila += '<td style="padding: 0rem" ><div style="margin: 0;" class="btn btn-app col-12 button-calendar" style="margin: 0;" ></div></td>';
				}
				
				for (let j = diainicio; j <= 6; j++) {
					fila += '<td style="padding: 0rem"> <div class="btn btn-app col-12 button-calendar" style="margin: 0;" data-toggle="modal"    data-target="#exampleModalForm" onclick="seleccionardia(' + Number(i).toFixed(0) + ')"> <span class="badge bg-danger" style="right: -1px; top: -1px">' + _datares[i].turno + "</span> " + Number(i).toFixed(0);
					
					if (_datares[i].clase != "") {
						fila +=	'<div><div class="external-event ' + _datares[i].clase + '" ></div>';
					}
				...
				for (let j = 0; j <= 6; j++) {
					if (i <= ultimoDia.getDate()) { //getDate obtiene el número de días del mes
            fila += '<td style="padding: 0rem"> <div class="btn btn-app col-12 button-calendar" style="margin: 0;" data-toggle="modal"     data-target="#exampleModalForm" onclick="seleccionardia(' +
              Number(i).toFixed(0) +
...
function cargarcalendario(_idusuario, _title) {
  ...
  $("#titleCalendario").text(" de " + g_persoanlSeleccionado.toUpperCase());
  $("#tablacontenedor tbody").empty();
  let nromes = $("#idmes").val();
      ...
      for (let i = 1; i <= ultimoDia.getDate(); i++) {
        ...
        if (i == 1) {
          for (let j = 0; j < diainicio; j++) {
            fila +=
              '<td style="padding: 0rem" ><div style="margin: 0;" class="btn btn-app col-12 button-calendar" style="margin: 0;" ></div></td>';
          }
          for (let j = diainicio; j <= 6; j++) {
            fila +=
              '<td style="padding: 0rem"> <div style="margin: 0;" class="btn btn-app col-12 button-calendar" style="margin: 0;" data-toggle="modal"    data-target="#exampleModalForm" onclick="seleccionardia(' +
              Number(i).toFixed(0) +
              ')"> <span class="badge bg-danger" style="right: -1px; top: -1px">' +
              _datares[i].turno +
              "</span> " +
            ...
            if (_datares[i].clase != "") {
              fila +=
                '<div><div style="margin: 0;" class="external-event ' +
                _datares[i].clase +
                '"></div>';
            }
            fila += "</div></div></td>";
        ...
        _datares.forEach((elemento, indice) => {
          $("#tablaProgramado tbody").append(
            "<tr>" +
              ...
              "')" +
              '">' +
              elemento.nombrecompleto.toUpperCase() +
              "</a></td>" +           
              "<td>" +
              elemento.horas +
              "</td>" +
  ...
  $.ajax({
    ...
    success: function (r) {
      if (r.success == true) {
        ...
        // msgbox(r.mensaje, "");
        msgbox("La programación se realizó correctamente");
...
function eliminarProgramacion(nro) {
  ...
  Swal.fire({
    title: '¿Deseas eliminar la programación?',
  ...
  //           //console.log(r.dataResult);
  //           // msgbox(r.mensaje, "");
  //           Swal.fire('¡Programación eliminada correctamente!', '', 'success')
  //           listadoPersonal();
  //           cargarcalendario($("#cmbpersonal").val(), g_persoanlSeleccionado);
  ...
  //     $("#exampleModalForm").modal("hide");
  //     Swal.fire('No se eliminó la programación', '', 'info')
  //   }
  // })
